...
import { pluralise, readingTime } from "../util/format.ts";
...
function renderRow(a: Article): string {
  ...
  return [
    ...
    `    <span class="article-meta">${when} \u00b7 ${pluralise(a.commentCount, "comment", "comments")}` +
      ` \u00b7 ${a.readingTime ?? readingTime(a.wordCount ?? 0)} min read</span>`,
